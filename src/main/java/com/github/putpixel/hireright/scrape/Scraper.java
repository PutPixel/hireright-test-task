...
public class Scraper {
	...
	private static final String[] TAGS_TO_IGNORE_ARR = { "area", "base", "br", "col", "embed", "hr", "img", "input",
	...
	private static List<String> doScrape(StringBuilder html) {
			...
			if (TAGS_TO_IGNORE.parallelStream().anyMatch(it -> tag.tag.toLowerCase().contains(it))) {
				...
				if (!startTag.tag.equals(endTag.tag)) {
					throw new IllegalArgumentException(
							"Incorrect html: Closing tag:" + endTag + ", opening tag: " + startTag);
				}
